...
class Sale_suspended extends CI_Model
{
	...
	function save ($items,$customer_id,$employee_id,$comment,$senior,$take_type,$payments,$sale_id=false)
	{
		if(count($items)==0)
			return -1;
	...
	function get_senior($sale_id)
	{
		$this->db->from('sales_suspended');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get()->row()->senior;
	}
	
	function get_take_type($sale_id)
	{
		$this->db->from('sales_suspended');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get()->row()->take_type;
	}
}
?>
