...
class Receiving extends CI_Model
{
	...
	function save ($items,$supplier_id,$employee_id,$comment,$senior,$take_type,$payment_type,$receiving_id=false)
	{
		if(count($items)==0)
			return -1;
		...
		foreach($items as $line=>$item)
		{
			...
			(
				...
				'trans_senior'=>$recv_remarks,
				'trans_take_type'=>$recv_remarks,
	...
	public function create_receivings_items_temp_table()
	{
		...
		(SELECT date(receiving_time) as receiving_date, ".$this->db->dbprefix('receivings_items').".receiving_id, comment,senior,take_type,payment_type, employee_id, 
