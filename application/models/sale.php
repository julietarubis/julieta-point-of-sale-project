...
class Sale extends CI_Model
{
	...
	function save (/*$or_id,*/$items,$customer_id,$employee_id,$comment, $senior, $take_type,$payments,$sale_id=false)
	{
		if(count($items)==0)
			return -1;
		...
		foreach($items as $line=>$item)
		{
			...
			(
				...
				'trans_senior'=>$sale_remarks,
				'trans_take_type'=>$sale_remarks,
			...
			//$or_id = $this->or_id->get_info($or_id);
	...
	public function create_sales_items_temp_table()
	{
		...
		(SELECT date(sale_time) as sale_date, ".$this->db->dbprefix('sales_items').".sale_id,comment,senior,take_type,payment_type, customer_id, employee_id, 
		...
		discount_percent, (item_unit_price*quantity_purchased*(100/112)) as subtotal,
		".$this->db->dbprefix('sales_items').".line as line, serialnumber, ".$this->db->dbprefix('sales_items').".description as description,
		ROUND((item_unit_price*quantity_purchased-((item_unit_price*quantity_purchased)*(100/112)*.12)-((item_unit_price*quantity_purchased)*(100/112))*discount_percent/100)*(1+(SUM(percent)/100)),2) as total,
		ROUND((item_unit_price*quantity_purchased*(100/112)-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(percent)/100),2) as tax,
		(item_unit_price*quantity_purchased-(item_unit_price*quantity_purchased*(100/112)*.12)-(item_unit_price*quantity_purchased*(100/112)*discount_percent/100) - (item_cost_price*quantity_purchased)) as profit
