...
class Sales extends Secure_area
{
	...
	function set_senior() 
	{
 	  $this->sale_lib->set_senior($this->input->post('senior'));
	}
	
	function set_take_type() 
	{
 	  $this->sale_lib->set_take_type($this->input->post('take_type'));
	}
	...
	function complete()
	{
		...
		$senior = $this->sale_lib->get_senior();
		$take_type = $this->sale_lib->get_take_type();
		...
		$data['sdiscount'] = $this->sale_lib->get_discount_sc();
		
		//$or_id = rand(10000000,99999999);
		$data['sale_id']= ''.$this->Sale->save(/*$or_id,*/$data['cart'], $customer_id,$employee_id,$comment,$senior,$take_type,$data['payments']);
	...
	function save($sale_id)
	{
		$sale_data = array(
			...
			'take_type' => $this->input->post('take_type')
		);
	...
	function _reload($data=array())
	{
		...
		$data['senior'] = $this->sale_lib->get_senior();
		$data['take_type'] = $this->sale_lib->get_take_type();
	...
	function suspend()
	{
		...
		//SAVE sale to database
		$data['sale_id']='POS '.$this->Sale_suspended->save(/*$or_id,*/$data['cart'], $customer_id,$employee_id,$comment,$take_type,$senior,$data['payments']);
		if ($data['sale_id'] == 'POS -1')
		{
			$data['error_message'] = $this->lang->line('sales_transaction_failed');
		}
