...
class Sale_lib
{
	...
	function get_senior() 
	{
		return $this->CI->session->userdata('senior');
	}
	
	function get_take_type() 
	{
		return $this->CI->session->userdata('take_type');
	}
	
	
	/*function get_senior_details() 
	{
		return $this->CI->session->userdata('senior_details');
	}
	
	function set_senior_details($senior_details) 
	{
		$this->CI->session->set_userdata('senior_details', $senior_details);
	}
	
	function clear_senior_details() 	
	{
		$this->CI->session->unset_userdata('senior_details');
	}*/
	...
	function set_senior($senior) 
	{
		$this->CI->session->set_userdata('senior', $senior);
	}
	
	function set_take_type($take_type) 
	{
		$this->CI->session->set_userdata('take_type', $take_type);
	}
	...
	function clear_senior() 	
	{
		$this->CI->session->unset_userdata('senior');
	}
	
	function clear_take_type() 	
	{
		$this->CI->session->unset_userdata('take_type');
	}
	...
	function copy_entire_suspended_sale($sale_id)
	{
		...
		$this->set_senior($this->CI->Sale_suspended->get_senior($sale_id));
		$this->set_take_type($this->CI->Sale_suspended->get_take_type($sale_id));
		//$this->set_senior_details($this->CI->Sale_suspended->get_senior_details($sale_id));
	...
	function clear_all()
	{
		...
		$this->clear_comment();
		$this->clear_senior();
		$this->clear_take_type();
		//$this->clear_senior_details();
	...
	function get_taxes()
	{
		...
		foreach($this->get_cart() as $line=>$item)
		{
			...
			foreach($tax_info as $tax)
			{
				...
				$tax_amount=($item['price']*$item['quantity']-($item['price']*$item['quantity']*(100/112)*.12)-$item['price']*$item['quantity']*$item['discount']/100)*(($tax['percent'])/100);
	...
	function get_subtotal()
	{
		...
		{
			//SENIOR FORMULA
			/*if(($item['discount']/100) == 0.2)
			{
			
			$pricevat = ($item['price']*$item['quantity']);
			$pwv = ($pricevat*(100/112));
			$srdc = ($item['discount']/100);
			$rfd = $pwv * $srdc;
			$subtotal += $pwv - $rfd;
			...
			//REGULAR FORMULA
			$subtotal+=($item['price']*$item['quantity']-($item['price']*$item['quantity']*(100/112)*.12)- $item['price']*$item['quantity']*$item['discount']/100);
			//$subtotal +=0;
			}*/
		    ...
		    $subtotal+=($item['price']*$item['quantity']-($item['price']*$item['quantity']*(100/112)*.12));
		}
		return to_currency_no_money($subtotal);
	}
	
	function get_discount_sc()
	{
		$sdiscount = 0;
		foreach($this->get_cart() as $item)
		{

		    $sdiscount+= (($item['price']*$item['quantity']-($item['price']*$item['quantity']*(100/112)*.12))*$item['discount']/100);
		}
		return to_currency_no_money($sdiscount);
	}
	...
	function get_total()
	{
		...
		foreach($this->get_cart() as $item)
		{
			...
			//$total += $subtotal;
            $total += ( ($item['price'] * $item['quantity'] - ($item['price']*$item['quantity']*(100/112)*.12))- $item['price'] * $item['quantity'] * $item['discount'] / 100);
			}*/
			
			$total += ( ($item['price'] * $item['quantity'] - ($item['price']*$item['quantity']*(100/112)*.12))- $item['price'] * $item['quantity']*(100/112) * $item['discount'] / 100);
