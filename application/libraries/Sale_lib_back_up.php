...
class Sale_lib
{
	...
	//	$vat_sales=$this->get_vat_sales();
		//$amount_due=to_currency_no_money($sales_total - $payment_total + $service_charge);
		
		//$subtotal = $this->get_payments_total();
		//$sale_sr = $this->get_sale_sr();
	//	$vat_exempt = $this->get_vat_exempt();
		//$sr_disc = $this->get_sr_disc();
	//	$serv_charge=$this->get_service_charge();
		//$amount_due=to_currency_no_money(($subtotal - $sale_sr)+($sale_sr - $vat_exempt - $sr_disc) + $serv_charge);
		//$totals = ($subtotal - $sale_sr)+($sale_sr - $vat_exempt - $sr_disc) + $serv_charge;
		return $amount_due;
	}
	
	//juliet sr. citizen
	function get_sale_sr()
	{
		$sale_sr = 0;
		$guest =5;
		$subtotal = $this->get_payments_total();
		$sale_sr= to_currency_no_money($subtotal/$guest);
		return $sale_sr;
	}
	...
	function get_service_charge()
	{
		//$service_charge=0;
		//$sales_total=$this->get_total();
		//$service_charge=to_currency_no_money($sales_total *0.08);
		
		$serv_charge = 0;
		$subtotal = $this->get_payments_total();
		$sale_sr = $this->get_sale_sr();
		$serv_charge= to_currency_no_money(($subtotal - $sale_sr)*0.107);
		return $serv_charge;
	}
	
		
	function get_vat_sales()
	{
		$vat_sales=0;
		$sales_total=$this->get_total();
		$vat_sales=to_currency_no_money($sales_total /1.12);
		return $vat_sales;
	}
	
	function get_vat_exempt()
	{
		$vat_exempt=0;
		$sale_sr = $this->get_sale_sr();
		$vat_exempt=to_currency_no_money(($sale_sr/1.12)*0.12);
		return $vat_exempt;
	}
	
	
	function get_sr_disc()
	{
		$sr_disc=0;
		$sale_sr=$this->get_sale_sr();
		$sr_disc= to_currency_no_money(($sale_sr/1.12)*0.20);
		return $sr_disc;
	}
	...
	function get_total()
	{
		...
		foreach($this->get_cart() as $item)
		{
           /* $total += ( $item['price'] * $item['quantity'] - $item['price'] * $item['quantity'] * $item['discount'] / 100); juliet comment*/
